...
test("DMM sampleから本棚カードを抽出する", () => {
  ...
  assert.match(book.detailUrl, /^https:\/\/book\.dmm\.com\/product\//);
...
test("DMM個別ページから購入済み巻だけを既存シリーズへ追加する", () => {
  const rule = rules.find((r) => r.id === "dmm-books-product");
  const url = "https://book.dmm.com/product/dmmseries999/dmmvol005/";
...
test("CSVはExcel向けBOM、固定列、引用符エスケープを持つ", () => {
  const csv = core.toCsv([{ source: "DMM", title: '題名, "特別"', statuses: ["完結"], manualStatuses: ["お気に入り"], ownedVolumes: [1,2,3,5,7,8,9] }]);
  assert.equal(csv.charCodeAt(0), 0xfeff);
  assert.match(csv, /"題名, ""特別"""/);
  assert.match(csv, /"サービス","シリーズID","書籍ID一覧"/);
  assert.match(csv, /"完結; お気に入り"/);
...
test("手動ステータスは自動ステータスと合算し、再収集マージ後も維持する", () => {
  const previous = { key: "x", statuses: ["完結"], manualStatuses: ["お気に入り"], externalIds: [], ownedVolumes: [] };

...
function rowHtml(r) {
  ...
  return `<tr><td class="favorite-cell"><button class="favorite-button${r.favorite ? " active" : ""}" data-favorite-key="${esc(r.key)}" aria-label="${r.favorite ? "お気に入りから外す" : "お気に入りに追加"}" title="${r.favorite ? "お気に入りから外す" : "お気に入りに追加"}">${r.favorite ? "★" : "☆"}</button></td><td><div class="book-cell">${cover}<div class="book-meta"><a class="book-title" href="${esc(r.detailUrl)}" target="_blank" rel="noreferrer">${esc(r.title)}</a><span class="book-authors">${esc(r.authors || "著者情報なし")}</span></div></div></td><td><span class="source-label">${esc(r.source)}</span></td><td class="volumes">${esc(EbookCore.formatVolumes(r.ownedVolumes) || "—")}</td><td><div class="badges">${badges}</div></td><td class="dates"><span>追加 ${esc(displayDate(r.firstSeenAt || r.lastSeenAt))}</span><span>確認 ${esc(displayDate(r.lastSeenAt))}</span></td><td><button class="icon-menu-button" data-row-menu-key="${esc(r.key)}" aria-haspopup="menu" aria-label="操作メニュー">⋮</button></td></tr>`;
}
